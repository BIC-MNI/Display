...
\begin{description}
\item[\menu{Prev}]  Sets the current object to the previous
item in the object list.
\item[\menu{Next}]  Sets the current object to the next item
in the object list.
\end{description}
...
axes. If a time axis is present, the intensity plot may also show the
timecourse of the current cursor position or an ROI. The
\menu{Intensity Plot} menu contains commands used to manipulate some of
the options for displaying this data.
...
\begin{description}
\item[\menutwo{Intensity Plot}{Enabled: On}] Toggles the intensity plot
 on or off. If for some reason you do {\em not} wish to see intensity
 plots, they can be disabled with this menu option.

\item[\menutwo{Intensity Plot}{Scaled: On}] If this option is {\bf on},
 the vertical axis of the intensity plot will be automatically scaled
 to the actual data range of the currently selected data. If this
 option is {\bf off}, the vertical axis will be permanently set to
 the full range of the voxel intensities of this volume.

\item[\menutwo{Intensity Plot}{Axis: Auto}] This option allows to you
 specify a preferred choice for the horizontal axis in the intensity
 plot. This command allows you to cycle through the choices {\tt
  Auto}, {\tt X}, {\tt Y}, and {\tt Z}. The time dimension, {\tt T},
 is also available for time-varying scans. The {\tt Auto} mode uses
 the logic described above to set the horizontal axis based on the
 dimensions of the loaded volume and the position of the mouse
 cursor.

If one of the specific axes is selected, that axis will be used as the
horizontal axis in the intensity plot irrespective of the current
location of the mouse cursor. However, if a measurement line is drawn,
the intensity along that line will still be displayed on the intensity
plot axis.
...
The Quit menu's main function is to allow users to explicitly
...
\item[\menutwo{Slice View}{Curr Volume}] If more than one volume is loaded, then
  this button increases the current volume index. If the the last volume is currently selected, this will return to the index of the first volume. The current volume index is displayed in the lower left corner of the slice view, with the field title {\tt V\#}. For many operations, including intensity plots and colour-coding selection,  the current volume is used as the target volume.
\item[\menutwo{Slice View}{Prev Volume}] If more than one volume is loaded, then
  this button decreases the current volume index. If this command is chosen when the first volume is selected, the current volume index will be set to the last volume loaded.
...
\item[\menutwo{Slice View}{Toggle Plane Visibility}] This menu selection
  toggles the visibility of the arbitrary (oblique) cross section plane in
  the 3D View window. This gives an alternate 3D view of the arbitary cross
  section plane.
...
\item[\menutwo{Slice View}{Toggle Slice Crs-Sect}]  Turns on or off the 
visibility of the projection of the arbitrary cross-section plane in the
three views which are {\em not} the current arbitrary view.
...
\item[\menutwo{Slice View}{Make All Visible}] Make all of the volumes visible again.
\item[\menutwo{Slice View}{Vol Opacity:}]  Prompts the user for an opacity
  value and sets the opacity of the current volume.
\item[\menutwo{Slice View}{Move to top}] Moves the currently selected volume to the ``top'' of the volume list, so that it will be drawn on top of other loaded volumes.
\end{description}
...
\subsubsection{Label visibility}

The visibility of individual labels can be toggled on or off. There
are two commands that support this. The first,
\menutwo{Segmenting}{Show/Hide Labels}, prompts for a specific label
number. The second, which is accessed by pressing the {\tt 'L'} key
while in the \menu{Segmenting} menu, affects only the {\em current} paint
label. In either case, if the label is currently visible, it will be
made invisible, and if it is currently invisible, it will be made visible.

Note that when painting operations are performed (e.g. with the right
mouse button), the current label will always be re-enabled.
...
This menu may be used to translate the position of all labels by
integral voxel increments.
...
This menu is used to perform various operations on lines objects,
including tractography data. Lines objects consist of a set of one or
more curves, each of which is defined by one or more line segments.

\begin{description}
\item[\menutwo{Lines}{Set Line Width}] Sets the width used for drawing
 the selected lines object.
\item[\menutwo{Lines}{Spline Line}] Converts a lines object to a new set of
 lines using spline interpolation. Each segment is smoothed to contain
 the number of segments defined using the \menutwo{3D Render}{Set \#
 Curve Segments} command.
\item[\menutwo{Lines}{Smooth Lines}] Prompts for a smoothing distance
  and attempts to smooth the current lines object.
\item[\menutwo{Lines}{Create Tube}] Create ``tubes'', or quadmeshes,
 along the tracks of the lines object. Prompts for the number of points
 in the circumference of the tubes, and the radius of the tubes.
\item[\menutwo{Lines}{Convert Markers}] Transforms the markers contained
  within the current model into an open curve. If you specify
  zero for the number of points, the curve will be created using
  the markers without interpolation. If you specify two or more for the
  number of points, cubic interpolation will be used to create a curve
  containing exactly as many points as requested.
\item[\menutwo{Lines}{Make Line Circle}] Creates a circle or ellipse
 using user-supplied parameters consisting of three centre
 coordinates, an axis index (0=X, 1=Y, or 2=Z), a width and height,
 and a number of segments.
\item[\menutwo{Lines}{Subdivide Line}] Doubles the number of points used
  to define a curve or line segment, by splitting each segment in half.
\item[\menutwo{Lines}{Convert Closed Marker}] Transforms the markers contained
  within the current model into a closed, segmented curve. If you specify
  zero for the number of points, the curve will be created using
  the markers without interpolation. If you specify two or more for the
  number of points, cubic interpolation will be used to create a curve
  containing exactly as many points as requested.
\item[\menutwo{Lines}{Colourize by XYZ}] Apply colours to the line
  segments according to their direction, with X=red, Y=green, and
  Z=blue.
\item[\menutwo{Lines}{Threshold by length}] Create a new lines object
  from the current lines object, by removing all lines that are less
  than a particular value in length.  
\end{description}
...
been scanned into a digital format and can be overlaid on any volume
...
\item[\menutwo{3D View}{Proj: Parallel/Perspective}]  Toggles between a
...
\item[\menutwo{3D View}{Type-in Cursor Pos}] Prompts the user to enter
  the cursor position in world coordinates.
\item[\menutwo{3D View}{Print View}] Prints the current view parameters
on the command-line terminal.
\item[\menutwo{3D View}{Front Plane}]  Not for general use.
\item[\menutwo{3D View}{Back Plane}]  Not for general use.
\item[\menutwo{3D View}{Toggle Stereo}]  Not for general use. Stereo
 mode is not properly implemented.
\item[\menutwo{3D View}{Eye Width}]  Not for general use.
\item[\menutwo{3D View}{Pick View}]  Not for general use.
\item[\menutwo{3D View}{Set Persp Distance}]  Not for general use.
\item[\menutwo{3D View}{Set Eye}]  Not for general use.
\item[\menutwo{3D View}{Set Window Width}]  Not for general use.
\item[\menutwo{3D View}{Set Line of Sight}]  Not for general use.
\item[\menutwo{3D View}{Set Up Dir}]  Not for general use.
\end{description}

\subsection{Objects}

This menu operates on the objects in the 3D window, which are also
listed in the Object List window. The object list is a hierarchical list
consisting of lines, polygons, markers, and collections of these, called
{\em models}. Generally, object operations apply to the currently
selected object which is the one which has a green rectangle around its
textual representation in the object list. Selection of the current
object can be performed by using the arrow keys to navigate the
hierarchy, or by clicking on the desired object text in the Object list
window. Another way to select the current object is to click on its
image in the 3D window.
...
\item[\menutwo{Objects}{Change Surface Prop}] Prompts for an ambient
  coefficient (0--1), a diffuse coefficient (0--1), a specular
  coefficient (0--1), a specular exponent (0--100 or so), and an opacity
  (near 0 is transparent, 1 is fully opaque).  The currently selected
  object is assigned these lighting parameters.
...
\item[\menutwo{Objects}{Scan Object to Volume}]  Causes the intersection
     ...
     markers.
\item[\menutwo{Objects}{Show Vertices}] Creates a marker for every point on
 a surface. You probably don't want to do this!
\end{description}

\subsection{3D Render}

This menu contains commands to changing the way 3D objects are
displayed in the 3D view window.

\begin{description}
\item[\menutwo{3D Render}{Change Background}] Prompts for a colour to
  set the background colour of the 3D window and slice window.
\item[\menutwo{3D Render}{Mode: Shaded}] Toggles the
  display mode of the current model between a wireframe rendering, point rendering, shaded (solid) rendering, and wireframe overlay (wireframe superimposed on shaded). The default is shaded rendering.
\item[\menutwo{3D Render}{Shading: Gouraud or Flat}] Toggles the
  display mode of the current object between a flat or smooth shading.
  The default is smooth (Gouraud).
\item[\menutwo{3D Render}{Lights: On or Off}] Toggles the lights on and off.  If
  lights are off, all objects are coloured uniformly.
\item[\menutwo{3D Render}{2 Sided}]  Not for general use.
\item[\menutwo{3D Render}{Backface}]  Not for general use.
\item[\menutwo{3D Render}{Set \# Curve Segments}] Sets the number of
  curve segments used for spline interpolation.
\item[\menutwo{3D Render}{Marker Labels: Off or On}] Toggles the
  display of the text labels of the markers in the 3D window.
\item[\menutwo{3D Render}{Lines as Curves: Off or On}] Toggles whether
  lines are drawn straight or curved. May not be supported on OpenGL.
\end{description}

\subsection{Markers}

This menu contains commands for creating and manipulating
markers. Markers are graphical objects that define a specific point in
the volume. Each marker can be associated with a number of pieces of
metadata, such as a label, a colour, a structure ID, and a patient ID.

Markers are now displayed both in the slice view and the 3D view
window. Markers will appear as outlines superimposed on the slice
view, or as 3D shaded objects in the 3D view. You can disable the
display of markers in the slice view by setting the global variable
{\tt Show\_markers\_on\_slice}. Individual markers can be turned on or off 
using the the \menutwo{Objects}{Toggle Visible} command.
...
\item[\menutwo{Markers}{Default Patient Id}]  Prompts the user to type in the
  default patient id.
\item[\menutwo{Markers}{Default Type}]  Toggles the current 
  default marker type between cube and sphere.
...
\item[\menutwo{Markers}{Chg Marker Type}] Toggles the current marker's
type between cube and sphere, if the current object is a marker.
\item[\menutwo{Markers}{Chg Marker Colour}] Prompts the user to type in the
  new colour of the current marker, if the current object is a marker.
...
Surface-based editing have recently been added to \display{}. In this
mode, the user can stretch and deform a surface in 3-D to take a new
shape.  It is a very efficient way of correcting voxel-based annotations
that define a 3-D surface, as an opposed to correcting the voxel-based
labels in each of the 3 planes.

The surface is loaded in \display{} together with the volume and 
possibly labels. After editions, the modified surface must be saved 
using \menutwo{File}{Save File}.

\begin{description}
\item[\menutwo{Polygons}{Print Surface Area}] Computes and prints the surface area of the currently selected polygonal object.
...
\item[\menutwo{Polygons}{Make Quad Ellipsoid}]  Same as
     ...
     the ellipsoid.
\item[\menutwo{Polygons}{Subdivide Polygons}] Subdivides a polygon by
  splitting each polygonal element into four polygons at the midpoints
  of the line segments. Will also ``upsample'' any vertex data
  associated with the polygonal surface.
\item[\menutwo{Polygons}{Create Bintree}]  Creates a ``bintree'', similar to a KD-tree, that can speed up certain operations on polygons.
...
\item[\menutwo{Surface Segmentation}{Cut Neighbours}] TBD
...
\item[\menutwo{Surface Segmentation}{Reset Neighbours}] TBD
...
\item[\menutwo{Surface Segmentation}{Paint Invisible}]  Paints the group of faces near the mouse cursor with the invisible colour, and makes them invisible. The numberof faces is determined by the current paint size.

\item[\menutwo{Surface Segmentation}{Paint Visible}] Paints the group of faces near the mouse cursor with the visible colour, and makes them visible. The numberof faces is determined by the current paint size.

\item[\menutwo{Surface Segmentation}{Vis Col:}] Prompts the user to enter a new colour to use for the visible colour.

\item[\menutwo{Surface Segmentation}{Invis:}] Prompts the user to enter a new colour to use for the invisible colour.

\item[\menutwo{Surface Segmentation}{Connected Vis Colour}] Sets all connected polygonal faces to the current visible colour.

\item[\menutwo{Surface Segmentation}{Connected Invis Colour}] Sets all connected polygonal faces to the current invisible colour.

\item[\menutwo{Surface Segmentation}{Paint Vis Colour}] Paints the group of faces near the mouse cursor with the visible colour. The number of faces is determined by the current paint size.

\item[\menutwo{Surface Segmentation}{Paint Invis Colour}] Paints the group of faces near the mouse cursor with the invisible colour. The number of faces is determined by the current paint size.

\item[\menutwo{Surface Segmentation}{Paint size:}] Prompts the user to enter a new paint size, the number of polygons (faces) to change on each paint operation.


\end{description}

\subsection{Volume Transform}
\label{secVolXfm}
...
\begin{description}
\item[\menutwo{Volume Transform}{Prev Visible}] Select the previous volume and make it visible.
\item[\menutwo{Volume Transform}{Scale Down Volume}] Transform the volume by dividing its size by the current scale step.
\item[\menutwo{Volume Transform}{Scale Up Volume}] Transform the volume by multiplying its size by the current scale step.
\item[\menutwo{Volume Transform}{Insert Volume as Labels}] Insert a selected volume as the label volume of the currently selected volume.
\item[\menutwo{Volume Transform}{Visible: 1}] Makes only one volume visible, cycling through all loaded volumes as the button is pressed.  Whichever volume
 is made visible is set to the current volume.
\item[\menutwo{Volume Transform}{Xform Volume Frm File}] Prompts the user for a transform file name, and then transforms the current volume accordingly.
\item[\menutwo{Volume Transform}{Reset Vol Xform}] Resets the volume transform to its original state, that is, the transform specified in the file itself.
\item[\menutwo{Volume Transform}{Trans Step: 1}] Prompts the user to enter a new value for the translation step, in world units.
\item[\menutwo{Volume Transform}{Rot Step: 1}] Prompts the user to enter a new value for the rotation step, in degrees.
\item[\menutwo{Volume Transform}{Save Volume Xform}] Prompts the user for a filename and saves the current volume transform as a {\tt .xfm} file.
\item[\menutwo{Volume Transform}{Scale Step: 1.05}] Prompts the user to enter a new value for the scale step, as a fraction of the current size.
\item[\menutwo{Volume Transform}{Trans + X}] Translates the volume towards the positive X axis using the current translation step.
\item[\menutwo{Volume Transform}{Trans + Y}] Translates the volume towards the positive Y axis using the current translation step.
\item[\menutwo{Volume Transform}{Trans + Z}] Translates the volume towards the positive Z axis using the current translation step.
\item[\menutwo{Volume Transform}{Rot + X}] Rotates the volume clockwise around the X axis using the current rotation step.
\item[\menutwo{Volume Transform}{Rot + Y}] Rotates the volume clockwise around the Y axis using the current rotation step.
\item[\menutwo{Volume Transform}{Rot + Z}] Rotates the volume clockwise around the Z axis using the current rotation step.
\item[\menutwo{Volume Transform}{Trans - X}] Translates the volume towards the negative X axis using the current translation step.
\item[\menutwo{Volume Transform}{Trans - Y}] Translates the volume towards the negative Y axis using the current translation step.
\item[\menutwo{Volume Transform}{Trans - Z}] Translates the volume towards the negative Z axis using the current translation step.
\item[\menutwo{Volume Transform}{Rot - X}] Rotates the volume counterclockwise around the X axis using the current rotation step.
\item[\menutwo{Volume Transform}{Rot - Y}] Rotates the volume counterclockwise around the Y axis using the current rotation step.
\item[\menutwo{Volume Transform}{Rot - Z}] Rotates the volume counterclockwise around the Z axis using the current rotation step.
\end{description}
...
\begin{enumerate}
\item Start \display{} as usual with a volume: 
\begin{verbatim}
Display prefix_104_t1_final.mnc 
\end{verbatim}
...
Within \display{} you can create a cut-away view of the cortical surface to show anatomical features inside. Type the following to open an average model brain and model of brainstem and cerebellum:
