...
\newcommand{\menu}[1]{{\sc\bf #1}\ }
\newcommand{\menutwo}[2]{{\sc\bf #1/#2}\ }
\newcommand{\menuthree}[3]{{\sc\bf #1/#2/#3}\ }
...
\item[Tags:]  contains a list of 3D points tagged on a volume.  This format
             ...
             lists of points, positions, etc.  usually ends in {\tt .tag}.
\end{itemize}

\section{\display windows}

\postscriptfig{display.ps}{7in}{7in}{Clockwise from top left:
3D window, slice window, menu window, text entry window \label{windows}}

Figure \ref{windows} shows the four windows created and used by \display :
...
such as {\tt 0.3 0.7 0.7} may be typed in.  The following is a brief
...
user, and are not described herein.
...
arrow keys are used to move around in the this hierarchy and to select the
current object.
...
\begin{itemize}
\item[Left Button]  Points to a position on a surface or marker.
\item[Middle Button]  Rotates, translates, or magnifies the view, depending
                      on the menu button pressed.
\end{itemize}

\section{Slice Window}
