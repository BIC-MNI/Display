...
such as \ident{red}, \ident{yellow}, or \ident{pink}, or a numerical colour,
...
\item[\menutwo{File}{Load File}]  Prompts for a filename and loads the file.
        If a volume file is loaded (ends in \ident{.mnc}), then the
...
\item[\menutwo{Create Surface}{Extracing}]  If a surface extraction
...
\item[\menutwo{Create Surface}{Make Perm}]  If a surface extraction
...
\item[\menutwo{Create Surface}{Volume Bin-Isosurf}]  Prompts for two
    ...
    the user should use \menutwo{Create Surface}{Volume Isosurface}.
...
\item[\menutwo{3D View}{Film Loop}]  Not for general use.
\item[\menutwo{3D View}{Save Image}]  Not for general use.
...
\item[\menutwo{Markers}{Save Mrkrs as .tag}]  Prompts the user to type in a
        ...
        \display or the \ident{register} program.  If the current object is
...
\item[\menutwo{Polygons}{Compute Normals}]  Computes normals for the
     ...
     the 3D window. 
\item[\menutwo{Polygons}{Average Normals}]  Prompts for a number of
...
\item[\menutwo{Polygons}{Set Line Thickness}]  Prompts for a line thickness
     ...
     Default value is 1.
\item[\menutwo{Polygons}{Scan Polygons to Volume}]  Causes the intersection
