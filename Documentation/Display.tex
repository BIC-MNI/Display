...
\item[3D window:]  contains the 3D objects, such as surfaces, lines,
                  and markers,
\item[slice window:]  shows up to 4 views of slices through one or more volumes
...
\item[Colour Bar]  The limits of the colour bar for the current volume
...
\item[\menutwo{File}{Load Labels .mnc}]  Prompts for a filename and
        loads the volume file as the label volume for the current slice window
        volume.
        The volume loaded must be exactly the same size as the current slice
        window volume.
\item[\menutwo{File}{Save Labels .mnc}]  Prompts for a filename and
        saves the label volume of the current volume.
...
\item[\menutwo{File}{Save Colour Map}]  Prompts for a filename and
        saves the current label colour map as an ascii file, with a default
        suffix of \ident{.map}.
...
\item[\menutwo{Slice}{Resample Volume}]  Prompts for 3 size parameter and
       resamples the current volume to the given size.
       Creates a smaller volume by box
...
\item[\menutwo{Slice}{Recompute Histogram}]  Creates a histogram of
       values in the current volume, which it displays in the slice window near
...
\item[\menutwo{Slice}{Toggle Plane Visib}]  
       This menu
       selection toggles the visibility of the current cross section plane in
       the 3D window.
...
\item[\menutwo{Slice}{Delete Volume}]  Brings up a submenu that allows the
       user to delete the current volume.
\item[\menutwo{Slice}{Volume Cropping}]  Brings up a submenu that allows the
       user to define a subregion of the volume and to load another volume
       cropped to the subregion.
\end{description}

\subsubsection{Volume Cropping Menu}
...
\item[\menuthree{Slice}{Volume Cropping}{Pick Crop Edge}]  After pressing this
       selection, an edge of the crop box in the slice window can be moved
       by pressing the left mouse button and dragging the edge to the desired
       position.
\item[\menuthree{Slice}{Volume Cropping}{Set Crop Filename}]  Prompts the
...
\item[\menutwo{Colour Coding}{Colour Code Object}]  Changes the
     ...
     coding parameters of the loaded volumes.
\end{description}

\subsubsection{Extra Colour Code Menu}

\begin{description}
\item[\menuthree{Slice}{Extra Colour Codes}{Red}]  Sets the colour
       coding method to range from black to full red.
\item[\menuthree{Slice}{Extra Colour Codes}{Green}]  Sets the colour
       coding method to range from black to full green.
\item[\menuthree{Slice}{Extra Colour Codes}{Blue}]  Sets the colour
       coding method to range from black to full blue.
\item[\menuthree{Slice}{Extra Colour Codes}{Slice Interp:}]
       Toggles the slice interpolation method among the choices of
       nearest neighbour (default), trilinear, and tricubic.  Note that the
       latter two, while producing smoother images, will cause the slice
       window to take many seconds or even a minute to update, depending
...
\item[\menutwo{Segmenting}{Set Paint Label}]  Sets the current label
    used for painting, where 0 erases.
\item[\menutwo{Segmenting}{XY Radius}]  Prompts for the in-slice 
    ...
    Any voxel whose centre is within the radius of the brush is painted.
    For this reason, any brush radius smaller than half the slice separation is
    treated as being half the slice separation.  A radius
    of zero is a special case, where, if any part of a voxel intersects the
    mouse, then the voxel is painted.  This gives the most fine control over
    painting.
...
\item[\menutwo{Segmenting}{Clear Fill}]  If the mouse is pointing
    ...
    flood fill algorithm.
\item[\menutwo{Segmenting}{Connectivity}]  Toggles between 8,26-neighbour
    and 4,6-neighbour connectivity in 2D,3D operations such as fill,
    dilate, and erode.  
...
This menu is used to create surfaces from the current volume.  Surfaces created
