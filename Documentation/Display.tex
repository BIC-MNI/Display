...
\item Input support for surface and geometric formats including Wavefront OBJ, Stanford (\ident{.ply}), GIFTI (\ident{.gii}), BrainSuite surfaces (\ident{.dfs}), BrainSuite tractography (\ident{.dft}), and FreeSurfer surfaces.
...
\item \ident{-vertex} $filename$ : Load per-vertex statistics from the given file and associate it with the most recently loaded surface. Data must be in \ident{.vertstats}, \ident{.csv}, \ident{.tsv}, or \ident{.txt} format.
...
\item[\menutwo{3D Render}{Set \# Curve Segments}]  Not for general use.
